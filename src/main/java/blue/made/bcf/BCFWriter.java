...
public class BCFWriter {
	...
	public class Array extends BCFWriter {
		...
		protected void preWrite(BCFType type) throws IOException {
			checkState();
			if (this.type != type) throw new IllegalStateException(type + " does not match the current type of this array (" + this.type + ")");
		}
	...
	protected void checkState() {
		if (atEnd) throw new IllegalStateException("The current BCF item has been completed, no more writes are possible");
		if (childInProgress) throw new IllegalStateException("A child of this item is in progress, it must be finished");
	}
